...
### 15.4 整个 3D 迁移研究弧的收束〔实验〕

> 残差**放对通道**（E3D-6）、**在对的分布上拟**（E3D-7）、**用对的结构装**（E3D-4b）——这三条在残差表达类**之内**成立；E3D-5 用外部 MuJoCo 划出表达类**之外**的边界：一步拟合是会骗人的代理，**拟合越好可能越有害，判别可迁移性只能靠外部留出的真实系统**。可微孪生 + 神经残差是强工具，但它的安全区由"失配是否落在残差表达类 ∩ 任务可控子空间"界定——这就是这条无人机→2D SRBM→3D Go2 研究弧给出的、边界清晰的句号。

## 16. 下一步（开放）

1. 更大表达类残差（关节力矩/广义力通道）能否触及腿惯量失配——E3D-5 边界的直接延伸。
2. body-frame 优化版接触换算；减面孪生（rendering LOD）支持批量短视野 BPTT。
3. 残差头质量工程（DAgger 迭代再采集、谱正则）+ 外部留出验证常态化。
