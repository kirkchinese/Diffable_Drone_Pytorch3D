...
**结论（部分验证，robustness>correction 方向成立）**：①**随机化覆盖的轴上 DR 确实收窄 gap**——调定点 kp300 DR 均值更优（86<92）且**方差减半（18→9）= 迁移更可靠**（不靠运气种子），kp400 明显更优（68<84）；全 kp **0 摔**（基础运动稳健跨到真机代理）。②**唯一回退在 kp200（低增益=执行器权限轴），恰是没随机化的轴**（孪生无 PD）——**DR 只在随机化过的轴上有效**，域随机的教科书规律，也精确指出补什么。③回答用户："可微 + 域随机"对 MuJoCo 代理真机**方向有效**（尤其调定点更稳更好），非 E3D-5 误读的"失败"；gap 不可消除但**可在随机化的轴上系统收窄**。直接下一步（§18 第6条）：补执行器增益/时延 DR 闭合 kp200，再叠感知(depth)+导航。
...
6. **补轴域随机闭合 kp200（接 E3D-9）**：当前 DR 只随机化了动力学参数（质量/惯量/足几何/接触），未覆盖**执行器权限/时延**轴→低增益 kp200 回退。加 action 低通/限幅/延迟随机化（模拟软/滞后驱动）应闭合 kp200；之后叠 depth 感知 + 导航避障，逐级过 MuJoCo 闸门再上真机。
