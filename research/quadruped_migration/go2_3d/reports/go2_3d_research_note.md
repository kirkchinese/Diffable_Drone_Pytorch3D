...
8. **摩擦的高牵引 binding 闸门（接 E3D-11）**：平地恒速测不出摩擦（E3D-11 反直觉:低μ反而更好/0摔）。须**坡度/加减速/急转/推扰**让牵引需求逼近 μ·f_n 上限——那才是打滑/摔的真 regime，也是 μ-域随机、摩擦感知（RGB 看冰面；heightmap/深度对摩擦盲）真正 binding 的地方；与 E3D-11b 摩擦×坡度合并做。
