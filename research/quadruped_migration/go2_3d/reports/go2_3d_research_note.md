...
## 16. E3D-8：自适应步态——从固定调度到自涌现，梯度保真是闸门〔实验〕

> 配套：[`dynamics/gait_adaptive.py`](../dynamics/gait_adaptive.py)（相位作状态的足端几何 + 耦合 CPG）、[`scripts/e3d8_gate.py`](../scripts/e3d8_gate.py)、[`scripts/e3d8b_cpg.py`](../scripts/e3d8b_cpg.py)、`results/e3d8_*.json`、`figures/e3d8_*.png`。
>
> 动机：E3D-4 的固定 trot（period/duty/PHASE_OFF 写死、相位由 t_step 闭式给出）不是省事，**是个梯度选择**——接触调度对参数平滑可微，正是绕开 2D-F4 的 1/dt 摩擦梯度爆炸、绕开接触模式切换不可微的根本手段。"自适应步态"最朴素的实现把"何时触地/离地"交给硬事件，等于把离散接触请回梯度图。**真正的问题不是"能不能训出自适应"，而是"自适应的参数化保不保梯度保真"**——这是整条线主题（梯度保真 > 前向拟合）在**调度层**而非残差层的复现。

### 16.1 闸门：连续相位自适应保不保梯度保真？〔实验〕

把相位提升为连续状态 φ、由瞬时频率 ω 推进（足端体系速度按 φ̇ 解析缩放，频率变→扫腿速度自动跟着变，摩擦速度伺服推进仍成立；配套自洽关系 Lx0=vx_cmd·duty/ω）。三臂唯一变量=步态参数化，策略架构同一：`fixed`（常 ω+平滑接触，≡E3D-4a 基线）/`phaseA`（策略调 ω+平滑接触）/`hard`（策略调 ω+硬接触切换，负控）。判据**不需 oracle**——自适应参数化引入的失真，用损失面的**有限差分真梯度 vs 解析 BPTT 梯度的一致性**直接测。

| 臂 | ①全程BPTT \|∇θL\|(H=800,中位) | ②cos(解析∇, FD真∇) H=50/150/300 |
|---|---|---|
| `fixed`（参照） | 1.7（有界） | **1.000 / 1.000** / 0.825 |
| `phaseA`（自适应 ω） | 3.5（有界） | **1.000 / 1.000** / 0.854 |
| `hard`（负控） | **∞（爆）** | **−0.56 / −0.09 / −0.06**（rel→1e13） |

**结论：连续相位的频率调制在 TBPTT 工作区（H≤150）梯度保真到机器精度、与固定基线无异；硬接触切换如 E3D-4a 主题预测彻底崩坏。** 两臂在 H=300 一同回落（fixed 0.825/phaseA 0.854）是长视野轨迹发散淹没（E3D-7 教训，正是 TBPTT≤150 为工作区的原因），非参数化缺陷。**判据兑现：自适应步态可达，当且仅当调度保持连续。** 这把"梯度保真"的管辖从残差通道推广到了步态参数化本身。

### 16.2 步态自涌现的机制：耦合 → 锁相 → 步态〔推导/实验〕

不写死 PHASE_OFF，给每腿一个相位振荡器经 Kuramoto 耦合 φ̇_i=ω+Σ_j K_ij·sin(2π(φ_j−φ_i)) 互相牵引，锁相后的模式即步态——由 K 的结构**对称破缺涌现**（不预设目标相位 Δ_ij）。纯振荡器（无身体）从随机初相积分：手设 trot-K 锁出 **[0,0.5,0.5,0]=精确 trot**（离 trot 0.000，lock 4e-9）；随机 K 锁出**别的**步态（[0,0,0.5,0]，离 trot 0.125）；零耦合仅冻结随机初相（不主动锁相）。机制成立——**能用 BPTT 训 K（步态本身），因耦合平滑可微（16.1 已证）**。

### 16.3 自适应频率训练：通道活、但此任务不受益〔实验〕

`fixed` vs `phaseA` 端到端训速度跟踪（H=600, TBPTT150, 70 iters）：

| 臂 | loss | vx RMSE | ω(学得/标称) |
|---|---|---|---|
| `fixed` | 0.028 | **2 mm/s** | 2.50/2.50（固定） |
| `phaseA` | 0.056 | 11 mm/s | **2.15**/2.50（周期内 ±0.02 调制） |

**诚实读数**：频率通道**活且梯度保真**——策略主动把 ω 压到 2.15（低 14%，更长支撑/步幅），并在步态周期内有节律地调制 ω，收敛干净无失稳（正如闸门承诺）。但在平地恒速这个**不需要自适应的任务上，频率调制不带来收益**（固定标称频率反而跟得更紧）。与全线一致的诚实：**额外表达力不免费；不需要它的任务不会奖励它**。这正是把自适应自由度移到**步态模式选择**（可能真有用）而非平地恒速的动机，也提示需换**真正要求自适应的任务**（变速/复杂地形）才能兑现价值。

### 16.4 可学习耦合端到端：步态会涌现，但 structure > free〔实验〕

把 K 作**可学习参数**端到端训速度任务（H=600, TBPTT150, 90 iters；BPTT 梯度保真由 16.1 保证）。两起点对照——随机小耦合（±1，无 trot 先验）vs trot 耦合起步（强 |4|）。读出**逐样本**相对 FL 的锁相偏移（圆均值；锁相度=平均合矢量长，1=锁死）：

| 起点 | loss | vx RMSE | 涌现偏移(相对FL) | 锁相度 | 离trot |
|---|---|---|---|---|---|
| 固定 trot（16.3 参照） | 0.028 | 2 mm/s | [0,.5,.5,0] 写死 | — | — |
| trot 耦合起步 | 0.054 | 14 mm/s | **[0,.5,.5,0] 精确 trot** | 1.00 | **0.000** |
| 随机耦合起步 | 0.096 | 35 mm/s | [0,.5,.02,1.0]（3+1 退化） | 0.91 | 0.122 |

两臂都训得动（loss 同步下降，闸门承诺兑现），但**涌现什么取决于归纳结构**：trot 耦合被**原样保留**（强耦合处对 K 的梯度小、K 几乎不动）→ 锁出精确 trot、步态可用；自由随机耦合**锁得牢（0.91）却锁到退化的 3+1 步态**（三腿近同相+一腿反相），RMSE 35 远逊。**梯度保真（训得动）是必要非充分；好步态需结构归纳偏置——自由耦合从零起步收到退化吸引子。这是 E3D-4b"结构化 4 参数胜自由 MLP"在步态拓扑层的复刻。** 诚实边界：随机起点单种子、90 iters、平地恒速；不排除更长训练/对称约束 K 能涌现更好步态，但当前证据是**朴素"让 K 可学"不自动给出 trot，强结构先验是关键**，与全线 structure>free 一致。

### 16.5 Tegotae 延伸臂：感觉反馈相位调制——条件性有值〔实验〕

相位推进被足端载荷调制 φ̇_i=(ω+耦合)·(1−σ·N_i)（N_i=归一化法向力，载重→慢推→久留支撑，感觉反馈 CPG）。对照开环 CPG（同预算同起点，唯一变量=反馈开关）：

| 设定 | 平稳 RMSE | 扰动后 RMSE | 备注 |
|---|---|---|---|
| 开环（trot 基） | 4 | 9 | 基线 |
| **Tegotae（trot 基, σ=0.3）** | 4 | **5** | 扰动恢复近 2× 更好（滑移持平） |
| Tegotae（退化 3+1 基, σ=0.6） | — | 93 | 灾难（相位近停滞、步态打爆） |

**诚实结论：感觉反馈条件性有值，不是免费午餐。** 稳定步态基底 + 温和增益下，载荷反馈平稳期中性、扰动后跟踪近 2× 更好（4.8 vs 8.5 mm/s）——兑现 Tegotae"载重久留支撑"的鲁棒性；但**增益过大或基底步态退化**时，载荷反馈把相位拖到近停滞、反而把步态打爆（96 mm/s）。其价值受基底步态质量与增益调参双重制约——与 16.4"结构决定成败"一脉相承。

### 16.6 E3D-8 收束：自适应步态可达、梯度保真可判，但"好"由结构与任务决定〔实验〕

> **①闸门**——相位作连续状态、策略调频率，梯度保真到机器精度（cos 1.00≈固定基线），硬接触切换如 E3D-4a 主题崩坏：**自适应步态可达，当且仅当调度保持连续**（梯度保真的管辖从残差通道推广到步态参数化本身）。**②涌现**——耦合振荡器锁相即步态，手设耦合给任意目标步态、可学习耦合端到端训得动（闸门保证），但自由耦合从零收到**退化吸引子**，唯结构先验（trot 耦合）保留并给好步态——**structure > free 在步态拓扑层复刻 E3D-4b**。**③通道兑现**——频率通道活但平地恒速不受益（不需要的自由度不被奖励）；载荷反馈在稳定基底+温和增益下助扰动恢复、否则有害（条件性）。
>
> **诚实总结**：可微 Go2 孪生**能**端到端学自适应/自选步态，且其可学性（梯度保真）能被 FD-vs-解析判据**确定性判定**——这是正面的方法学结果。但"涌现出好步态"不是自动的：自由参数化收到退化解、过激反馈打爆步态——**好步态由归纳结构（对称约束的耦合、稳定基底）与任务（要求自适应的地形/变速）共同决定**。与无人机→2D→3D 全弧一致：**梯度保真是地基，结构是承重墙**。直接下一步（§20 第5条）：对称群约束的耦合 K + 要求步态转换的变速/地形任务，验证"好步态自涌现"。

## 17. E3D-9：可微孪生 + 域随机 → 过 MuJoCo 闸门（robustness > correction）〔实验〕

> 配套：[`scripts/e3d9_domain_rand.py`](../scripts/e3d9_domain_rand.py)、`results/e3d9_gate.json`、`figures/e3d9_gate.png`；复用 E3D-5 的 `mj_go2` 桥 + `rollout_mj` 闸门。
>
> 动机（用户重定向，纠正 E3D-5 误读）：目标不是"无 sim2real gap"（不可能），而是"训练方法在一定程度还原真实、部署后保持不错性能"，**用 MuJoCo 当中间真机判据**。E3D-5 被误读为"可微路线失败"——其实 **nominal 孪生策略迁 MuJoCo 本就走起来了（92mm/s、0 摔）**，失败的只是用残差去**修正**孪生（越修越差）。正确的工具不是修正(correction)是**鲁棒(robustness)**：孪生上域随机 → 训对一整族孪生鲁棒的策略 → 过 MuJoCo → 上真机。可微梯度负责每实例样本效率，鲁棒来自随机化（"可微 + 域随机"混合路线，与 RL+DR 同源逻辑、引擎换 BPTT）。

**有的放矢的域随机（用 E3D-5 诊断 C_kin≫C_force：失配以运动学/惯量为主）**：足端几何（±2cm x / ±1.2cm z，bracket E3D-5 实测 δ̂）、逐轴惯量(×0.7–1.4)、质量(×0.85–1.15)、接触(k_n×0.5–2, μ×0.7–1.3)；3 DR 种子，H=600/iters=120。判据=MuJoCo 闸门跨 **PD 增益失配** kp∈{200,300,400}（300=E3D-5 调定点；200/400=真·执行器失配应力，孪生侧无此参数）。既有 nominal（H=800 训）作基线 → DR 赢即保守。

| kp（PD增益） | nominal RMSE | DR RMSE | 摔(各/3) |
|---|---|---|---|
| 200（软·低权限） | **123±5** | 136±8 | 0 / 0 |
| **300（调定点）** | 92±18 | **86±9（方差减半）** | 0 / 0 |
| 400（硬） | 84±17 | **68±13** | 0 / 0 |

**结论（部分验证，robustness>correction 方向成立）**：①**随机化覆盖的轴上 DR 确实收窄 gap**——调定点 kp300 DR 均值更优（86<92）且**方差减半（18→9）= 迁移更可靠**（不靠运气种子），kp400 明显更优（68<84）；全 kp **0 摔**（基础运动稳健跨到真机代理）。②**唯一回退在 kp200（低增益=执行器权限轴），恰是没随机化的轴**（孪生无 PD）——**DR 只在随机化过的轴上有效**，域随机的教科书规律，也精确指出补什么。③回答用户："可微 + 域随机"对 MuJoCo 代理真机**方向有效**（尤其调定点更稳更好），非 E3D-5 误读的"失败"；gap 不可消除但**可在随机化的轴上系统收窄**。

**E3D-9b 补执行器滞后轴（用户选"补 kp200"）〔实验〕**：给孪生加**执行器一阶滞后** DR 轴（realized 足端动作低通 a_filt=(1−β)a_filt+βa_raw，τ∈[0,30ms] 随机；孪生本无 PD，此为训练增广让策略对"动作没即时到位"鲁棒）。三臂跨 kp：

| kp | nominal | DR(动力学) | DR+act(加滞后) |
|---|---|---|---|
| 200(软) | 123±5 | 136±8 | 128±15 |
| 300(调定) | 92±18 | **86±9** | 86±10 |
| 400(硬) | 84±17 | **68±13** | **145±103** |
| 跨kp总均 | 100 | **96** | 120 |

**诚实结论（交叉折损，DR 非免费）**：加滞后轴**微改善目标 kp200（136→128）但未闭合（仍>nominal 123），代价是 kp400 回退+失稳（68→145±103）、净更差（96→120）**。机制：一阶滞后只是低 kp 失配（=柔顺+欠达+滞后）的**粗代理**——补了"慢"没补"欠达/柔顺"，又过度展宽 DR 伤了刚性端。**这把 E3D-9 的律推到更本质：DR 不是"随机化即覆盖"，而是"轴必须忠实建模真实机制"——粗代理/错形式的随机化，和粗残差(E3D-5)、自由耦合(E3D-8)一样会反噬。** dynamics-only DR（E3D-9）仍是最稳赢的鲁棒臂（总均 96、调定点方差减半）。

**E3D-9c 忠实可微 PD/柔顺执行器层（用户选"忠实建模执行器"）〔实验〕**：把 9b 的粗动作低通换成**忠实二阶临界阻尼跟踪**——realized 足端目标经 ẍ=ω_n²(x_cmd−x)−2ζω_n ẋ 跟踪 commanded（=well-tuned 关节 PD 响应:滞后+欠达；ω_n∝√kp，刚端 ω_n 大→近直通，本应不伤刚性端），随机化 ω_n∈[40,300] rad/s。四臂跨 kp：

| kp | nominal | DR(动力学) | DR+act(粗滞后) | DR+comp(忠实柔顺) |
|---|---|---|---|---|
| 200(软) | 123±5 | 136±8 | 128±15 | 136±10 |
| 300(调定) | 92±18 | **86±9** | 86±10 | 107±25 |
| 400(硬) | 84±17 | **68±13** | 145±103 | 99±34 |
| 跨kp总均 | 100 | **96** | 120 | 114 |

**诚实结论（执行器轴对 SRBD 孪生出表达类）**：忠实柔顺层比粗滞后稍好（kp400 99 vs 145 不失稳）但**仍未闭合 kp200（136=dynamics-DR，未追平 nominal 123），且回退调定点 kp300（86→107±25）**，净劣于 dynamics-DR。**两次补执行器轴（粗一阶滞后 9b / 忠实二阶柔顺 9c）都失败**，根因结构性：**SRBD 孪生无关节，PD 柔顺是关节空间 + 载荷相关，在足端笛卡尔层既建模不准（Jacobian 各向异性）、又随机化展宽伤调定点、且柔顺 rollout 更难训（欠拟合，s1/s2 loss 0.16–0.26）。即执行器/PD 轴对 SRBD 孪生出表达类**——与 E3D-5 多体惯量出类、E3D-8 步态结构同一条铁律：**表达类/结构是承重墙，DR 旋钮补不了出类的轴**。

**E3D-9 整线收束**：①**robustness>correction 在孪生能表示的轴上成立**——dynamics-DR（质量/惯量/足几何/接触）是 SRBD 孪生的正确鲁棒工具（总均 96、调定点方差减半、kp400 最优、0 摔），把"可微 + 域随机 → 过 MuJoCo 代理真机"证为**方向有效**。②**DR 不是万能旋钮**——出表达类的轴（执行器 PD 柔顺、多体惯量）补不了，要覆盖须**结构升级**（关节动力学/重建 ABA 栈），非随机化。③对"上真机"：dynamics-DR 策略可作里程碑部署（MuJoCo 调定点已更稳），但真机的软执行器区（≈kp200）是 SRBD 孪生的盲区，须靠真机自身 PD 调参或更全仿真。**这就是"可微端到端机械狗"feasibility 的实验级回答：可微+域随机能在孪生能表示的轴上系统收窄 sim2real（部署后保持不错性能成立），但孪生表达类外的轴（关节/接触多体）需结构升级或别的引擎——gap 不可消除、可在能表示的轴上管理、出类的轴须换更全模型。**

## 18. E3D-10：感知地形运动——Stage 2 第一砖（接感知）〔实验〕

> 配套：[`dynamics/terrain.py`](../dynamics/terrain.py)（平滑高斯地形 + 机体前向 heightmap）、[`scripts/e3d10_perceptive.py`](../scripts/e3d10_perceptive.py)、`results/e3d10_perceptive.json`、`figures/e3d10_perceptive.png`。
>
> 动机：用户重定向后第一步"接感知"。给策略一个**机体前向 heightmap**（便宜的可微外感知，proto-depth；full 深度图渲染成本高后置）+ **逐腿摆动高度**动作 dh，对照**盲（heightmap 置零）vs 感知**，看地形上感知是否摔更少/走更远。地形=平滑高斯隆起（C∞，守梯度保真，避尖锐台阶接触梯度爆炸）。

**结果（两个地形难度都没拿到干净的感知优势）**：① 平滑易地形（h 0.04–0.10、稀）：盲只摔 3%、感知 5%——**地形太宽容（平滑接触让足平顺骑过隆起），盲不失败 → 感知无优势可拿**（同 E3D-8"不需要的能力不被奖励"）。② 陡密难地形（h 0.05–0.11、密 10 隆起/1.9m）：**盲与感知双双崩**（盲摔 83–92%、感知 84%，一种子还梯度爆 |g|84 训练发散）。**梯度全程有界（|g|中位 0.2，平滑地形 + heightmap 可微 → 感知→控制回路梯度保真，这条新回路本身成立）。**

**诚实诊断（同一条"匹配机制"铁律）**：感知没赢不是"感知无用"，是**干预与失败模式不匹配**——`dh`（摆动高度）只能治"摆动足绊上孤立隆起"，但密集地形的主失败是"**支撑足踩在不平地形上把机体顶歪**"，摆高治不了；地形一难，主失败就从"绊"切到"支撑倾覆"，`dh` 失效 → 盲感知齐崩。与 E3D-5 残差通道、E3D-9 域随机轴同律：**干预必须对上真实失败机制**。要拿干净的感知优势：要么**稀疏孤立障碍**（让"绊"成为主失败，dh 恰好对症），要么**更富动作**（足端落点 + 姿态/降速），让感知能驱动对症的修正。

**E3D-10 的兑现**：感知→控制的**可微回路打通且梯度保真**（这是 Stage 2 的地基），但**最小动作（仅摆高）+ 反应式策略**不足以在崎岖地形兑现感知优势——下一砖须把动作对上失败机制（§20 第7条）。

## 19. E3D-11：摩擦扫描 MuJoCo 闸门——补机械狗最核心的轴〔实验〕

> 配套：[`scripts/e3d11_friction.py`](../scripts/e3d11_friction.py)、`results/e3d11_friction.json`、`figures/e3d11_friction.png`；复用 E3D-5 桥（`rollout_mj` 加 `ground_mu` 钩）。
>
> 动机（用户指出的盲区）：复杂地面/摩擦从没单独测。机械狗推进=支撑足摩擦速度伺服，μ 是运动最核心的轴；E3D-9 把 μ 捆在域随机里（×0.7–1.3）但从没隔离/压到打滑区。这里扫 MuJoCo 地面 μ∈{0.3,0.5,0.7,1.0,1.3}，现成 nominal vs dynamics-DR 评测（kp=300）。

| μ(MuJoCo) | nominal vx / RMSE | DR vx / RMSE | 摔 |
|---|---|---|---|
| 0.3 | 0.30 / **40** | 0.28 / 43 | 0/0 |
| 0.5 | 0.28 / 60 | 0.26 / 64 | 0/0 |
| 0.7 | 0.27 / 83 | 0.24 / 80 | 0/0 |
| 1.0 | 0.25 / 105 | 0.23 / 94 | 0/0 |
| 1.3 | 0.24 / 114 | 0.22 / 103 | 0/0 |

**反直觉结论（纠正"低 μ 打滑崩"的预测）**：平地恒速 trot 下 μ∈[0.3,1.3] **全程 0 摔**，且**低 μ 反而跟踪更好**（RMSE 40→114 随 μ 单调升）。机制：**软（低μ）接触滤掉落足误差、grippy（高μ）接触把落足误差放大成机体抖**。**摩擦在平地恒速不是失败轴**——牵引需求低，μ=0.3 也够推（不打滑失速）。dynamics-DR ≈ nominal（摩擦鲁棒在此用不上，μ-DR 既没帮也没伤）。

**方法论教训（同 E3D-10/E3D-8）**：要让摩擦成 binding constraint（真打滑/摔的 regime），须上**高牵引需求任务**——坡度（重力分量要摩擦抵）、加减速、急转、推扰恢复。**平地恒速测不出摩擦的重要性**（恰如 E3D-10 易地形测不出感知、E3D-8 平地测不出自适应）：**任务必须把该轴逼到 binding 才判得出**。这是有价值的纠偏：常识"低摩擦危险"只在高牵引时成立。直接下一步（§20 第8条）：摩擦×坡度/推扰的高牵引闸门（E3D-11b）——那才是 μ 与感知（RGB 看冰面；heightmap/深度对摩擦天然盲）真正 binding 的地方。

**E3D-11b：摩擦×坡度 MuJoCo 闸门——把摩擦逼到 binding（接 E3D-11；用户问"山地/斜坡能不能模拟"）〔实验〕**：配套 [`scripts/e3d11b_friction_slope.py`](../scripts/e3d11b_friction_slope.py)、`results/e3d11b_friction_slope.json`、`figures/e3d11b_friction_slope.png`；`rollout_mj` 加 `slope_deg` 钩（地板仍平=坡面对齐系，倾斜重力 g=(−g sinθ,0,−g cosθ)，+x 上坡——忠实建斜坡物理，无需地形几何）。上坡支撑摩擦既要推进、又要扛重力沿坡分量 mg·sinθ，可用切向=μ·mg·cosθ → **打滑硬判据 μ<tanθ**。扫 θ∈{0,8,16,24,30}°×μ∈{0.3,0.6,1.0}，现成 nominal vs DR（kp=300，平地训策略上坡=真 sim2real 问题）：

| μ | θ0 | θ8 | θ16 | θ24 | θ30 | 失速/打滑起始 | 理论 μ<tanθ |
|---|---|---|---|---|---|---|---|
| 0.3 | +0.30 | +0.16 | **−1.45**(滑退) | −4.66 | −7.61 | **16°** | 16.7° |
| 0.6 | +0.27 | +0.25 | +0.18 | −0.18 | −1.63 | 24° | 31° |
| 1.0 | +0.25 | +0.23 | +0.20 | ~0(卡住) | −0.04 | 24° | 45° |

**结论（摩擦在斜坡上确成 binding，与 E3D-11 平地非失败轴构成决定性对照）**：① **同坡度跨 μ 隔离出摩擦轴**——θ=16° 时 μ=0.3 **向后滑退**（vx −1.45，正是打滑）而 μ≥0.6 仍上爬（+0.18），低 μ 在更缓坡就崩、高 μ 撑更陡；μ=0.3 的崩点 16° 几乎精确命中 tanθ=16.7° 判据。② **两种失败模式可分**：低 μ=滑退（vx 大负、机体不倒），高 μ=策略不会爬坡而倾覆（"摔"多在 μ=1.0；平地训的策略没学上坡）——vx 符号把"打滑"与"策略无能"分开。③ **DR≈nominal**（μ-DR 0.56–1.04 没把坡度牵引轴随机化 → 不买上坡鲁棒，同 E3D-9 律）。④ **决定性方法学**：同一摩擦轴，平地"低 μ 反而最好/0 摔"、16°坡"低 μ 滑退"——**轴的重要性取决于任务是否把它逼到 binding**。⑤ 对用户"山地/斜坡能否模拟"：**斜坡物理现已在闸门里忠实建模并实测**（倾斜重力法），坡度可信。
...
5. **好步态自涌现（接 E3D-8）**：对称群约束的耦合 K（把四足步态对称性写进参数化，降自由度→破 16.4 的退化吸引子）+ **要求步态转换的任务**（速度扫描/复杂地形，让 trot↔walk↔bound 转换有收益）；闸门已证此路梯度保真，缺的是结构先验与对的任务。
6. **执行器轴是 SRBD 孪生的出表达类盲区（接 E3D-9b/9c，结论已收敛）**：粗一阶滞后（9b）与忠实二阶柔顺（9c）两次都没闭合 kp200——根因是 SRBD 无关节、PD 柔顺在关节空间，足端笛卡尔层 DR 补不了。两条路：(a) **结构升级**——重建关节动力学/ABA 孪生（足端目标→关节 PD→柔顺力，关节空间忠实），才能把执行器轴纳入可微+DR；(b) **接受边界**——dynamics-DR（E3D-9）已是孪生能表示轴上的最优鲁棒臂，直接上真机里程碑（真机软执行器区靠真机 PD 调参）+ 推进 depth 感知/导航避障。先 (b) 验证里程碑、(a) 视真机表现再定。
7. **感知优势兑现（接 E3D-10）**：当前仅"摆动高度"动作 + 反应式策略，对不上密集地形的"支撑倾覆"主失败。两条：(a) **稀疏孤立障碍**任务（让"绊"成主失败，dh 对症）作干净的感知正例；(b) **更富动作**（足端落点 + 姿态/降速）+ 可选记忆（让感知驱动对症修正）。之后再叠 MuJoCo 地形闸门（E3D-10b）验证感知策略 sim2real。
8. **摩擦的高牵引 binding（坡度已做=E3D-11b，余下推扰/感知）**：E3D-11b 已证斜坡上摩擦按 μ<tanθ binding（低μ滑退）。余下：(a) **μ-域随机下探到含打滑区（≤0.3）+ 在斜坡上重训**，看 DR 能否买上坡摩擦鲁棒（E3D-9 律在摩擦轴的验证）；(b) **摩擦感知须 RGB**——冰面/低μ斑块对 heightmap/深度天然盲（看着平），是用户"深度或RGB"里 RGB 的真正用武之地（空间变摩擦 + 看纹理减速/绕行）；(c) 推扰恢复、急转等其他高牵引轴。
9. **楼梯/尖锐台阶=可微孪生的出类边界**：尖锐竖直台阶=接触不连续=梯度爆炸（2D-F4/E3D-4a 承重墙），在平滑可微孪生里只能 sigmoid 近似。两条：(a) 训练侧用平滑近似 + MuJoCo 硬接触台阶作闸门验证（分工:孪生平滑训、MuJoCo 硬验）；(b) 台阶/攀爬这类强不连续任务本就更适合无模型 RL（可微的优势在平滑动力学）。
