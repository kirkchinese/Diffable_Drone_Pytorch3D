...
**诚实结论（交叉折损，DR 非免费）**：加滞后轴**微改善目标 kp200（136→128）但未闭合（仍>nominal 123），代价是 kp400 回退+失稳（68→145±103）、净更差（96→120）**。机制：一阶滞后只是低 kp 失配（=柔顺+欠达+滞后）的**粗代理**——补了"慢"没补"欠达/柔顺"，又过度展宽 DR 伤了刚性端。**这把 E3D-9 的律推到更本质：DR 不是"随机化即覆盖"，而是"轴必须忠实建模真实机制"——粗代理/错形式的随机化，和粗残差(E3D-5)、自由耦合(E3D-8)一样会反噬。** dynamics-only DR（E3D-9）仍是最稳赢的鲁棒臂（总均 96、调定点方差减半）。对"上真机"的直接指导：kp200/软执行器区需在孪生里**忠实建模 PD/柔顺**（而非粗滞后），或接受该轴用更全的仿真处理——这是 §18 第6条修正后的方向。
...
6. **忠实建模执行器闭合 kp200（接 E3D-9b，方向已修正）**：E3D-9b 证明粗一阶滞后 DR 不闭合 kp200 且伤 kp400（轴须忠实建模）。正确做法=在孪生里加**真 PD/柔顺执行器层**（足端目标→关节 PD→柔顺力，可微），把"软驱动"作为忠实的动力学项而非动作低通代理，再随机化其增益；之后叠 depth 感知 + 导航避障，逐级过 MuJoCo 闸门再上真机。
