...
class DroneTrainer:
    ...
    def __init__(self, args):
        ...
        if args.no_odom:
            ...
            self.model = Model(dim_obs=10, dim_action=6).to(self.device)  # 7 + 3 (local_v)
    ...
    def run_episode(self, iteration):
        ...
        # 动作延迟缓冲 (模拟真实控制延迟)
        # 关键：参考项目使用 env.act 初始化，长度为 act_lag + 1
        ...
        # 目标位置 (随机生成)
        p_target = torch.rand(B, 3, device=self.device) * 18.0 - 9.0  # [-9, 9] 范围
        target_v_raw = p_target - self.env.p
        
        # 个体随机化最大速度 (参考项目逻辑: 0.75 + 2.5 * rand)
        ...
        for t in range(args.timesteps):
            ...
            # 解析输出: [加速度向量(3), 速度预测(3)]
            # act_raw: (B, 6) -> reshape to (B, 3, 2) for R @ ...
            act_reshaped = act_raw.reshape(B, 2, 3).permute(0, 2, 1)  # (B, 3, 2)
            act_world = R_local @ act_reshaped  # 转换到世界坐标系
    ...
    def train(self):
        ...
        for i in pbar:
            ...
            if (i + 1) % 10000 == 0:
                save_path = os.path.join(args.save_dir, f'checkpoint_{(i+1)//10000:04d}.pth')
