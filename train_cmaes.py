"""
CMA-ES + Gradient dual-loop training script for differentiable drone navigation.

This script extends the standard gradient-based training with CMA-ES evolutionary
optimization for a DecayController that modulates gradient decay per sample.
"""
...
from model import Model_bigger, DecayController
...
def parse_args():
    """解析命令行参数（继承自 train.py 并添加 CMA-ES 参数）"""
    parser = argparse.ArgumentParser(description='无人机避障训练脚本（CMA-ES + 梯度双重优化）')
    ...
    # CMA-ES parameters
    parser.add_argument('--cma_pop_size', type=int, default=20, help='CMA-ES population size')
    parser.add_argument('--cma_sigma0', type=float, default=0.5, help='CMA-ES initial step size')
    parser.add_argument('--cma_eval_interval', type=int, default=50, help='CMA-ES evaluation interval (training steps)')
    parser.add_argument('--decay_min', type=float, default=0.2, help='Minimum decay factor')
    parser.add_argument('--decay_max', type=float, default=1.0, help='Maximum decay factor')
...
class CMAESTrainer:
    ...
    def __init__(self, args):
        ...
        self.decay_controller = DecayController(
            ...
            decay_range=args.decay_max - args.decay_min
        ).to(self.device)
        
        # CMA-ES optimizer for decay controller
        x0 = self.decay_controller.get_params_vector().cpu().numpy()
        cma_options = {
            'popsize': args.cma_pop_size,
            'seed': 42,
            'maxiter': 1e6,  # effectively infinite, we control iterations
            'verbose': -1,   # suppress cma's own printing
        }
        self.cma_es = cma.CMAEvolutionStrategy(x0.tolist(), args.cma_sigma0, cma_options)
        ...
        log_path = os.path.join(args.log_dir, f'drone_train_cmaes_{timestamp}')
    ...
    def run_episode(self, iteration, decay_controller=None):
        """
        运行一个 episode 并返回损失
        
        Args:
            iteration: 当前迭代编号
            decay_controller: 可选的 DecayController 实例，用于动态梯度衰减
            
        Returns:
            tuple: (loss, metrics, debug_data, extra)
                - loss: 标量损失
                - metrics: 指标字典
                - debug_data: 调试数据 (p_history, v_history, act_buffer, vid)
                - extra: 额外数据用于 CMA-ES 评估
        """
        args = self.args
        B = args.batch_size
        ...
        for t in range(args.timesteps):
            ...
            # 确保 depth 是有效的张量
            if depth is None:
                depth = torch.zeros((B, args.image_height, args.image_width), device=self.device)
            # 重新启用梯度（深度图作为模型输入）
            ...
            # 模型推理
            act_raw, img_feat, h = self.model(x, state, h)
            
            # 计算衰减因子
            decay = None
            if decay_controller is not None:
                with torch.no_grad():
                    decay = decay_controller(img_feat.detach())  # (B,)
        ...
        if decay_history:
            extra['decay_history'] = decay_history
        
        return loss, metrics, debug_out, extra
    
    def compute_fitness(self, p_history, p_start, p_target, vec_to_obj_history, margin):
        """
        ...
        - 不前进但存活: progress≈0 → fitness≈0
        """
        total_dist = (p_target - p_start).norm(dim=-1)  # (B,)
        final_dist = (p_target - p_history[-1]).norm(dim=-1)  # (B,)
        progress = (1.0 - final_dist / (total_dist + 1e-6)).clamp(0, 1)  # (B,)
        
        # 碰撞检测: 任意步中距离 < margin 即碰撞
        distance = vec_to_obj_history.norm(dim=-1) - margin  # (T, B)
        collision = (distance < 0).any(dim=0).float()  # (B,)
        
        # 平均速度
        if p_history.shape[0] > 1:
            avg_speed = (p_history[1:] - p_history[:-1]).norm(dim=-1).mean(0)  # (B,)
        else:
    ...
    def evaluate_cma_individual(self, params_vector):
        ...
        self.decay_controller.set_params_vector(
            torch.tensor(params_vector, dtype=torch.float32, device=self.device)
        )
        with torch.no_grad():
            _, metrics, _, extra = self.run_episode(iteration=-1, decay_controller=self.decay_controller)
    ...
    def train(self):
        ...
        cma_gen = 0  # CMA-ES 代数计数
        
        for i in pbar:
            try:
                # 1. 正常梯度训练步（使用当前 best decay controller）
                loss, metrics, debug_data, extra = self.run_episode(i, self.decay_controller)
            ...
            # 记录 decay 统计
            with torch.no_grad():
                if 'decay_history' in extra:
                    decay_vals = torch.stack(extra['decay_history'])  # (T, B)
                    metrics['decay_mean'] = decay_vals.mean().item()
                    metrics['decay_std'] = decay_vals.std().item()
                    metrics['decay_min_val'] = decay_vals.min().item()
                    metrics['decay_max_val'] = decay_vals.max().item()
            
            # 2. CMA-ES 更新（每 N 步）
            if (i + 1) % args.cma_eval_interval == 0 and not self.cma_es.stop():
                # 保存当前最优参数
                ...
                for sol in solutions:
                    try:
                        ...
                        gc.collect()
                        torch.cuda.empty_cache()
                        fitnesses.append(0.0)  # OOM 个体给 0 fitness（最小化中的中性值）
                        print(f"\n[OOM] CMA-ES eval OOM, assigning neutral fitness")
                ...
                if best is not None:
                    self.decay_controller.set_params_vector(
                        torch.tensor(best, dtype=torch.float32, device=self.device)
                    )
                else:
                    # 第一代可能还没有 best，恢复之前的
                ...
                metrics['cma_sigma'] = self.cma_es.sigma
                metrics['cma_generation'] = cma_gen
                
                print(f"\n[CMA-ES] gen={cma_gen}, best_fit={best_fitness:.4f}, "
                      f"mean_fit={mean_fitness:.4f}, sigma={self.cma_es.sigma:.4f}")
            ...
            if i >= 200 and self.ar_ema > self.best_ar:
                ...
                torch.save({
                    ...
                    'decay_controller': self.decay_controller.state_dict(),
                }, best_path)
                # 同时记录到 TensorBoard
        ...
        # 保存最终模型
        self._save_checkpoint(args.num_iters - 1, is_final=True)
    ...
    def _save_checkpoint(self, iteration, is_final=False):
        """保存模型 + DecayController + CMA-ES 状态"""
        suffix = 'final' if is_final else f'{iteration+1:06d}'
        save_path = os.path.join(self.args.save_dir, f'checkpoint_{suffix}.pth')
        
        checkpoint = {
            'model': self.model.state_dict(),
            'decay_controller': self.decay_controller.state_dict(),
            'cma_es_mean': self.cma_es.mean.tolist(),
            'cma_es_sigma': self.cma_es.sigma,
            'iteration': iteration,
        }
...
def main():
    args = parse_args()
    print("Training arguments (CMA-ES mode):")
    print(args)
